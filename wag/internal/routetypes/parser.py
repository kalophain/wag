...
from .key import Key
from .policy import Policy, PolicyType, ANY, PUBLIC, RANGE, SINGLE, DENY, TCP, UDP, ICMP
...
def parse_rules(
    ...
    public: List[str],
    deny: List[str]
) -> Tuple[List[Rule], List[Exception]]:
    """
...
def validate_rules(mfa: List[str], public: List[str], deny: List[str]) -> Optional[str]:
    """
...
def acls_to_routes(rules: List[str]) -> Tuple[List[str], Optional[Exception]]:
    """
...
def _parse_rule(restriction_type: PolicyType, rule: str) -> Rule:
    ...
    if len(parts) == 1:
        ...
        values = [Policy(
            policy_type=restriction_type | SINGLE,
            proto=ANY,
            lower_port=ANY
        )]
    else:
...
def _parse_port_range(lower_port: str, upper_port: str, proto: str) -> Policy:
    ...
    if lower_port_num > upper_port_num:
        raise ValueError(
            f"lower port cannot be higher than upper port: lower: {lower_port} upper: {upper_port}"
        )
    ...
    if proto == "any":
        return Policy(
            policy_type=RANGE,
            proto=ANY,
            lower_port=lower_port_num,
            upper_port=upper_port_num
        )
