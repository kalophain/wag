...
class Policy:
    ...
    def __init__(self, policy_type: int = 0, proto: int = 0, lower_port: int = 0, upper_port: int = 0):
        """
        Initialize a policy.
    ...
    def to_bytes(self) -> bytes:
        ...
        """
        return struct.pack(
            "<HHHH", self.policy_type, self.proto, self.lower_port, self.upper_port  # 4 unsigned shorts, little-endian
        )
    ...
    def from_bytes(self, data: bytes) -> None:
        ...
        self.policy_type, self.proto, self.lower_port, self.upper_port = struct.unpack("<HHHH", data[:8])
    ...
    def __str__(self) -> str:
        ...
        if self.is_type(RANGE):
            return (
                f"{restriction_type}({self.policy_type}) {self.lower_port}-{self.upper_port}/{self._lookup_protocol()}"
            )
    ...
    def __eq__(self, other) -> bool:
        ...
        return (
            ...
            and self.lower_port == other.lower_port
            and self.upper_port == other.upper_port
        )
